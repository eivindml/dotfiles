...
module.exports = function generateCode(node, codegen) {
    ...
    if (properties) {
        var objectProps = Object.keys(properties).map(propName => {
            return builder.property(builder.identifier(propName), properties[propName]);
        });

        node.setAttributeValue('data-marko', builder.objectExpression(objectProps), false);
    }
    ...
    if (isCustomElement && attributes && attributes.length) {
        propertiesScript = builder.functionCall(codegen.context.helper('propsForPreviousNode'), [builder.objectExpression(attributes.map(attr => builder.property(builder.identifier(attr.name), attr.value))), builder.identifier('out')]);
    }

    if (bodyOnlyIf) {
        var startIf = builder.ifStatement(builder.negate(bodyOnlyIf), [startTag]);

        var endIf = builder.ifStatement(builder.negate(bodyOnlyIf), [endTag, propertiesScript]);
